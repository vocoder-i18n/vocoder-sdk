...
server.tool(
  ...
  {
    sourceLocale: z.string().optional().describe('Source language code (default: "en")'),
    targetLocales: z.array(z.string()).optional().describe('Target language codes, e.g. ["es", "fr", "de"]'),
  },
  async ({ sourceLocale, targetLocales }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `Setup detection failed: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  },
);
...
server.tool(
  ...
  async () => {
    ...
    if (!client) return { content: [{ type: 'text', text: NO_API_KEY_MESSAGE }] };
    try {
      const text = await runStatus(client);
      return { content: [{ type: 'text', text }] };
    } catch (error) {
      return { content: [{ type: 'text', text: `Status check failed: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  },
);
...
server.tool(
  ...
  {
    branch: z.string().optional().describe('Git branch to sync (auto-detected from git if not provided)'),
    force: z.boolean().optional().describe('Force re-sync even if strings are unchanged'),
    mode: z.enum(['auto', 'required', 'best-effort']).optional().describe('Sync mode: "auto" (default), "required" (block until done), "best-effort" (queue and return immediately)'),
  },
  async ({ branch, force, mode }) => {
    ...
    try {
      ...
      return { content: [{ type: 'text', text: `Sync failed: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  },
);
...
server.tool(
  ...
  {
    branch: z.string().optional().describe('Branch to fetch translations for (default: "main")'),
    locale: z.string().optional().describe('Specific locale to return (e.g. "es"). Returns all locales if omitted.'),
  },
  async ({ branch, locale }) => {
    const client = createClient();
    if (!client) return { content: [{ type: 'text', text: NO_API_KEY_MESSAGE }] };
    try {
      ...
      return { content: [{ type: 'text', text: `Failed to fetch translations: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  },
);
...
server.tool(
  ...
  async () => {
    ...
    try {
      ...
      return { content: [{ type: 'text', text: `Failed to list locales: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  },
);
...
server.tool(
  ...
  {
    locale: z.string().describe('BCP 47 locale code to add, e.g. "fr" or "pt-BR"'),
  },
  async ({ locale }) => {
    const client = createClient();
    if (!client) return { content: [{ type: 'text', text: NO_API_KEY_MESSAGE }] };
    try {
      ...
      return { content: [{ type: 'text', text: `Failed to add locale: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  },
);
