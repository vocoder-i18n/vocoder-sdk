...
export class VocoderClient {
  ...
  async sync(body: SyncBody): Promise<TranslationBatchResponse> {
    return this.request<TranslationBatchResponse>('POST', '/api/cli/sync', body);
  }

  async getSyncStatus(batchId: string): Promise<TranslationStatusResponse> {
    return this.request<TranslationStatusResponse>('GET', `/api/cli/sync/status/${batchId}`);
  }
  ...
  async getSnapshot(
    ...
    return this.request<TranslationSnapshotResponse>('GET', `/api/cli/sync/snapshot?${params}`);
  }
  ...
  async listLocales(): Promise<{ locales: Array<{ code: string; name: string; nativeName?: string }> }> {
